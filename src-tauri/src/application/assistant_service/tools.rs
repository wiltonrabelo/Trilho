...
pub(super) fn parse_args(raw: &str) -> Value {
    serde_json::from_str(raw).unwrap_or(json!({}))
}

pub(super) fn truncate_tool_text(mut out: String) -> String {
...
pub(super) fn path_arg(args: &Value) -> Result<String, String> {
    let path = args
        .get("path")
        .and_then(|p| p.as_str())
...
pub(super) fn paths_arg(args: &Value) -> Result<Vec<String>, String> {
    let arr = args
        .get("paths")
...
pub(super) fn commit_id_arg(args: &Value, key: &str) -> Result<String, String> {
    let id = args
        .get(key)
        .and_then(|p| p.as_str())
...
pub(super) fn stash_index_arg(args: &Value) -> Result<usize, String> {
    let idx = args
        .get("index")
        .and_then(|v| v.as_u64())
